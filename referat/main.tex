...
Gramatika rozpoznávající název modelu notebooku nepracuje s~žádným kontextem, v~rámci trénovací množiny dokumentů nebyl společný kontext nalezen (s~výjimkou možného levého kontextu \uv{Model:} v~dokumentu ze stránky ebay, takový kontext ovšem pokládáme za nedostatečně specifický). 
...
Další fáze slouží k~odstranění anotací u~chybně detekovaných řetězců. Například firma Microsoft vyrábí notebooky, ale i operační systém Windows. Pokud tedy detekovaný název notebooku obsahuje \uv{Windows}, je tato anotace odstraněna.
...
Dále je do gramatiky zařazené obecnější pravidlo, využívajícího pravého kontextu \uv{graphics card} nebo \uv{GPU}. Za GPU bude označený předcházející text o~3-5 tokenech, skládající se z~tokenů začínajících na velké písmeno, nebo skládajících se z~alfanumerických znaků (s~případnou pomlčkou).
